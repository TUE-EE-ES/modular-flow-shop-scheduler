...
def loadSummaries(file, extract_categories=False):
    df = pandas.DataFrame()
...
def runScheduler(input, output):
    ...
    if DRY_RUN == False:
        sys.stdout.flush()
        sys.stderr.flush()    
        if subprocess.call([hcssBinary, '-a', 'forward', '-i', input, '-o', output], stdout=sys.stdout, ) != 0:
...
def main(args):
    if(len(args)) != 4:
        print ("Expecting <input> <output> <lowerbounds>")
        raise Exception("Incompatible number of arguments; expected exactly two arguments")
    ...
    lb = loadSummaries(lb_files, extract_categories=False)
    ...
    out['optimal'] = out.apply(lambda x : x['lowerbound'] == x['Makespan'], axis=1 )
    out['% over lb'] = out.apply(lambda x : (x['Makespan']/x['lowerbound'] -1)*100 if x['lowerbound'] > 0 else np.nan , axis=1 )
    incorrect = out['incorrect'] = out.apply(lambda x : x['lowerbound'] > x['Makespan'], axis=1 )
    ...
    print ("*************************")
    print (out[out.optimal == False][['Benchmark', 'lowerbound', '% over lb', 'Makespan', 'Execution time']])
    ...
    if incorrect.sum(): # check if any is True (i.e. non-zero)
        ...
        print ("*************************")
        print (out[out.incorrect == True][['Benchmark', 'lowerbound', 'Makespan']]) # also includes the 'incorrect' column to show which ones are incorrect now.
