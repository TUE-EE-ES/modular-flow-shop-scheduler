...
ProductionLineWidget::ProductionLineWidget(FORPFSSPSD::ProductionLine productionLine,
                                           QWidget *parent) :
...
void ProductionLineWidget::open_bounds_clicked() {
    ...
    std::vector<FMS::ProductionLineSequences> sequences;
    for (const auto &jsonSequence : jsonSequences) {
        auto sequence =
                algorithm::Sequence::loadProductionLineSequences(jsonSequence, m_productionLine);
