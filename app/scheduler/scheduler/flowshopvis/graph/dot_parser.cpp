...
using namespace FlowShopVis::DOT;
using namespace DelayGraph;

void FlowShopVis::DOT::sanitizeAndAddStatement(const std::size_t start,
...
FlowShopVis::DOT::Statements FlowShopVis::DOT::splitIntoStatements(const std::string &graphString) {
...
std::string FlowShopVis::DOT::getFirstDigraph(const std::string &dotFile) {
    ...
    FMS::trim(graphString);
    return graphString;
}
std::tuple<StatementType, std::string, std::string, std::string, std::string>
FlowShopVis::DOT::getEdgeStrings(const std::string &statement) {
...
FlowShopVis::DOT::Option FlowShopVis::DOT::getOptionsKeyValue(const std::string &option) {
...
FlowShopVis::DOT::Options FlowShopVis::DOT::getStatementOptions(const std::string &options) {
...
std::optional<FORPFSSPSD::operation>
FlowShopVis::DOT::getOperationFromLabel(const std::string &label) {
...
std::optional<FORPFSSPSD::operation>
FlowShopVis::DOT::getOperationFromOptions(const Options &options) {
...
std::optional<delay> FlowShopVis::DOT::getEdgeWeightFromOptions(const Options &options) {
    for (const auto &[key, value] : options) {
        if (key == "weight") {
            try {
...
QColor FlowShopVis::DOT::getColourFromOptions(const Options &options) {
    for (const auto &[key, value] : options) {
        if (key == "color") {
...
DotFileResult FlowShopVis::DOT::parseDotFile(const std::filesystem::path &dotFilePath) {
    ...
    std::unordered_map<std::string, DelayGraph::VertexID> vertexMap;
    std::vector<std::tuple<std::string, std::string, delay, QColor>> edges;
    ...
    for (const auto &statement : statements) {
        ...
        if (type == StatementType::NODE) {
            ...
            if (vertexMap.find(src) == vertexMap.end()) {
                vertexMap[src] = result.graph.add_vertex(operation.value());
            }
        } else {
            // It's an edge, find the weight option
            std::optional<delay> weight = getEdgeWeightFromOptions(options);
            if (!weight.has_value()) {
                return {};
            }
    ...
    for (const auto &[src, dst, weight, color] : edges) {
        ...
        result.graph.add_edge(vSrc, vDst, weight);
