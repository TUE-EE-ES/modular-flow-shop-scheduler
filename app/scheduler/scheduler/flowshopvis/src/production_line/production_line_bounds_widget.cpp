...
void ProductionLineBoundsWidget::setBounds(std::vector<fms::problem::GlobalBounds> bounds) {

    const auto moduleIdsRepeated = bounds | std::views::join | std::views::keys;

    const std::unordered_set<fms::problem::ModuleId> moduleIdsSet(moduleIdsRepeated.begin(),
                                                                  moduleIdsRepeated.end());
    std::vector<fms::problem::ModuleId> moduleIds(moduleIdsSet.begin(), moduleIdsSet.end());
    std::ranges::sort(moduleIds.begin(), moduleIds.end());

    std::unordered_map<fms::problem::ModuleId, std::vector<fms::problem::ModuleBounds>>
            moduleBounds;
