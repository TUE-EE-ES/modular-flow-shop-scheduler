...
QVariant getBound(const fms::problem::IntervalSpec &bounds,
                  const fms::problem::JobId jobIdFrom,
                  const fms::problem::JobId jobIdTo,
                  const bool invert) {
...
void highlightChanged(const fms::problem::IntervalSpec &oldBounds,
                      const fms::problem::IntervalSpec &newBounds,
                      const fms::problem::JobId jobFrom,
                      const fms::problem::JobId jobTo,
                      std::size_t posX,
                      std::size_t posY,
...
void FlowShopVis::BoundsModel::setBounds(std::vector<fms::problem::ModuleBounds> bounds) {
    beginResetModel();
    m_bounds = std::move(bounds);
    m_currentIndex = 0;

    m_sortedJobs.clear();
    std::unordered_set<fms::problem::JobId> jobs;
