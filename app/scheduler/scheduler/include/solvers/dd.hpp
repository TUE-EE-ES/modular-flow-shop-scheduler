...
public:
    ...
    using WeakVertex = std::weak_ptr<DD::Vertex>;
    ...
    template <typename T>
    [[nodiscard]] static std::tuple<Solutions, nlohmann::json>
    solveWrap(FORPFSSPSD::Instance &problemInstance, const commandLineArgs &args);
    ...
    [[nodiscard]] static SharedVertex createNewVertex(std::uint64_t &vertexId,
    ...
    static std::vector<SharedVertex> expandVertex(std::uint64_t &vertexId,
    ...
    static SharedVertex mergeOperator(const DD::Vertex &a,
                                      const DD::Vertex &b,
                                      std::uint64_t &vertexId,
                                      const FORPFSSPSD::Instance &problemInstance,
                                      const DelayGraph::delayGraph &dg);
    ...
    static SharedVertex popQueue(std::deque<SharedVertex> &states);
    static SharedVertex popQueue(std::vector<SharedVertex> & /*states*/) {
        throw std::runtime_error("FmsScheduler::should only call this function for non static or "
                                 "non adaptive search");
    }

    template <class F>
    static SharedVertex popQueue(std::vector<SharedVertex> &states, F comparator);
    template <class F>
    static SharedVertex popQueue(std::deque<SharedVertex> & /*states*/, F /*comparator*/) {
        throw std::runtime_error(
                "FmsScheduler::should only call this function for static or adaptive search");
    }

    template <class V>
    static void pushQueue(std::vector<SharedVertex> &states, SharedVertex &s, V comparator);
    template <class V>
    static void
    pushQueue(std::deque<SharedVertex> & /*states*/, SharedVertex & /*s*/, V /*comparator*/) {
        throw std::runtime_error("FmsScheduler::should only call this function for static or "
                                 "adaptive or best search");
    }

    static void
    pushQueue(std::deque<SharedVertex> &states, SharedVertex &s, DDExplorationType comparator);
    static void pushQueue(std::vector<SharedVertex> & /*states*/,
                          SharedVertex & /*s*/,
    ...
    template <class V> static void orderQueue(std::vector<SharedVertex> &states, V comparator);
    template <class V>
    static void orderQueue(std::deque<SharedVertex> & /*states*/, V /*comparator*/) {
        throw std::runtime_error(
                "FmsScheduler::should only call this function for static or adaptive search");
    }

    template <typename T>
    static void push(T &states,
    ...
    template <typename T>
    static SharedVertex pop(T &states, DDExplorationType explorationType, DD::DDSolution &solution);
};
