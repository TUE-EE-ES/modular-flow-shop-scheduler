...
std::optional<delay> parseDefault(const xml_node<> *node) {
    ...
    if (pEnd == attr->value()) {
        LOG_W("Unrecognized default value '{}' for node '{}'", attr->value(), node->name());
    }
    return val;
}
...
SingleFlowShopParser::SingleFlowShopParser() :
    numberOfJobs(-1), m_defaultSheetSize(0), m_maximumSheetSize(-1) {}

problem::Instance SingleFlowShopParser::extractInformation(std::string_view fileName,
                                                           const xml_node<> *const root,
                                                           const cli::ShopType type) {
    ...
    return {stdfs::path(fileName).stem().string(),
            ...
            {std::move(sheetSizes), m_defaultSheetSize},
            m_maximumSheetSize,
            type};
}

void SingleFlowShopParser::loadMaintenancePolicy(problem::Instance &instance, std::string fname) {
...
problem::DefaultOperationsTime SingleFlowShopParser::loadProcessingTimes(const xml_node<> *pTimes) {
    ...
    for (xml_node<> *child = pTimes->first_node(); child != nullptr;
        ...
        processingTimes[problem::Operation{jId, oId}] = value;
    }
...
std::tuple<problem::DefaultTimeBetweenOps, problem::TimeBetweenOps>
SingleFlowShopParser::loadSetupTimes(const xml_node<> *root) {
    ...
    problem::DefaultTimeBetweenOps setupTimes{defaultSetupTime};
    problem::TimeBetweenOps setupTimesIndep;
...
void SingleFlowShopParser::loadSheetSizes(const xml_node<> *sSizes) {
    ...
    for (const auto *child = sSizes->first_node(); child != nullptr;
        ...
        sheetSizes[problem::Operation{jid, oid}] = value;
...
std::map<problem::MaintType, delay>
SingleFlowShopParser::loadMaintenanceProcessingTimes(const xml_node<> *const pTimes) {
    std::map<problem::MaintType, delay> maintDuration;

    // iterate over the processing times and sizes
    for (const auto *child = pTimes->first_node(); child != nullptr;
         child = child->next_sibling()) {

        const auto maintType = static_cast<problem::MaintType>(intAttribute(child, "t"));
        const auto value = static_cast<delay>(intAttribute(child, "value"));
...
std::map<problem::MaintType, std::tuple<delay, delay>>
SingleFlowShopParser::loadMaintenanceThresholds(const xml_node<> *const thresh) {
    std::map<problem::MaintType, std::tuple<delay, delay>> thresholds;
...
SingleFlowShopParser::loadTimeTable(const xml_node<> *const node, std::string_view subNodes) {
    ...
    for (const auto *child = node->first_node(subNodes.data(), subNodes.size()); child != nullptr;
        ...
        const auto value = static_cast<delay>(intAttribute(child, "value"));
        problem::Operation op1{jid1, oid1};
        problem::Operation op2{jid2, oid2};
...
PlexityTable SingleFlowShopParser::loadJobPlexity(const xml_node<> *const jp) {
    problem::PlexityTable jobPlexity;
    ...
    for (xml_node<> *child = jp->first_node(); child != nullptr; child = child->next_sibling()) {
        ...
        problem::ReEntrancies reEntrancies(2);
        ...
        if (value == "S") {
            ...
            try {
                reEntrancies = problem::ReEntrancies(std::stoi(value));
        ...
        const auto jobId = static_cast<problem::JobId>(intAttribute(child, "j"));
...
problem::Instance FORPFSSPSDXmlParser::createFlowShop(const cli::ShopType type) {
    SingleFlowShopParser p;
    return p.extractInformation(getFileName(), getFirstNode(), type);
}

problem::ProductionLine FORPFSSPSDXmlParser::createProductionLine(const cli::ShopType type) {
    const auto *const nodeRoot = getFirstNode();

    std::unordered_map<ModuleId, problem::Instance> modules;
    ...
    return ProductionLine::fromFlowShops(
            stdfs::path(getFileName()).stem().string(), modules, transfer);
}

problem::ModulesTransferConstraints FORPFSSPSDXmlParser::loadTransferPoints(
...
std::tuple<std::unordered_map<problem::JobId, delay>, std::optional<delay>>
