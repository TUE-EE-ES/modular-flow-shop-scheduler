...
#include "fms/solvers/geometric_selection_operator.hpp"
...
GeometricSelectionOperator::GeometricSelectionOperator(const unsigned int intermediate_solutions) :
    ...
    LOG_D("reduction to {}", intermediate_solutions);
}

std::vector<PartialSolution>
GeometricSelectionOperator::reduce(std::vector<PartialSolution> values) const {
    ...
    if (result.size() > intermediate_solutions) {
        throw std::runtime_error(std::string("Reduction operator did not reduce enough;")
                                 + std::to_string(values.size()) + " was reduced to "
                                 + std::to_string(result.size()) + " which is still larger than "
                                 + std::to_string(intermediate_solutions));
    }
    return result;
}

bool GeometricSelectionOperator::compareEntries(const PartialSolution &t1,
                                                const PartialSolution &t2) {
